...
export class Action {
  ...
  public async execute(input = {}): Promise<ActionInstance> {
    const raw = <Record<string, ActionInstanceDescription>>
            await this.device.client.post(this.device.actionsHref(), {[this.name]: {input: input}});
    return new ActionInstance(Object.values(raw)[0], this);
  }

  public async queue(): Promise<ActionInstance[]> {
    const raw = <Record<string, ActionInstanceDescription>[]> await this.device.client.get(this.href());
